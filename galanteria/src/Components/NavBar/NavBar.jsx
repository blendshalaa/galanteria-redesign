  ...
  useEffect(() => {
    const handleOutsideClick = (event) => {
      if (
        ...
        // setToggleBtn(false); // Close the menu when clicking outside
        document.body.classList.remove("scroll-y"); // Remove scroll lock
      }
    };
      ...
      <div className='left'>
        ...
        <ul onClick={() => toggleMenu()} className={`${toggleBtn ? "header-menu" : ""} links`}>
          ...
          <div className="categories" ref={categoriesRef}>  
            ...
            {showDropdown && (
              <ul className='dropdown'>
                <li>
                  <Link to='/category1' onClick={toggleHomeFurnitureList}>Home Furnitures</Link>
                  {showHomeFurnitureList && (
                    <ul className="nested-list">
                      <li>
                        <Link to='/category1'>Bedrooms</Link>
                      </li>
                      <li>
                        <Link to='/category1'>Bathrooms</Link>
                      </li>
                    </ul>
                  )}
                </li>
          ...
          <div className='ecatalog' ref={ecatalogueRef}>
            <a href={pdf} download>
              <svg className='d' width="20px" height="20px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
