import l1 from '../assets/images/LIGHTT.png'
import l2 from '../assets/images/LIGHT (2).jpg'
import l3 from '../assets/images/l3.jpg'

import l4 from '../assets/images/l4.jpg'
import l5 from '../assets/images/l5.jpg'
import l6 from '../assets/images/l6.jpg'
import l7 from '../assets/images/l7.jpg'
import l8 from '../assets/images/l8.jpg'
import l9 from '../assets/images/l9.jpg'
import l10 from '../assets/images/l10.jpg'
import l11 from '../assets/images/l11.jpg'
import l12 from '../assets/images/l12.jpg'
import l13 from '../assets/images/l13.jpg'
import l14 from '../assets/images/l14.jpg'
import l15 from '../assets/images/li15.jpg'
import g1 from '../assets/images/GIULIA.jpg'
import g2 from '../assets/images/g2.jpg'
import g3 from '../assets/images/g3.jpg'
import g4 from '../assets/images/g4.jpg'
import g5 from '../assets/images/g5.jpg'
import j1 from '../assets/images/JETTT.png'
import j2 from '../assets/images/jet1.png'
import j3 from '../assets/images/jet2.png'
import j4 from '../assets/images/jet3.png'
import j5 from '../assets/images/jet4.png'
import j6 from '../assets/images/jet5.png'
import k1 from '../assets/images/KINGGG.jpg'
import k2 from '../assets/images/K2.jpg'
import k3 from '../assets/images/KING.jpg'
import m1 from '../assets/images/MALIQE.png'
import m2 from '../assets/images/m1.png'
import m3 from '../assets/images/m2.png'
import q1 from '../assets/images/QUENNN.jpg'
import q2 from '../assets/images/QUEEN.jpg'
import q3 from '../assets/images/q1.jpg'
import q4 from '../assets/images/q2.jpg'
import q5 from '../assets/images/q3.jpg'
import kl1 from '../assets/images/kl1.jpg'
import kl2 from '../assets/images/kl2.jpg'
import kl4 from '../assets/images/kl4.jpg'
import kl5 from '../assets/images/kl5.jpg'
import kl6 from '../assets/images/kl6.jpg'
import kl7 from '../assets/images/kl7.jpg'
import kl8 from '../assets/images/kl8.jpg'
import kl9 from '../assets/images/kl9.jpg'
import kl10 from '../assets/images/kl10.jpg'
import kl11 from '../assets/images/kl11.jpg'
import kl12 from '../assets/images/kl12.jpg'
import kl13 from '../assets/images/kl13.jpg'
import kl14 from '../assets/images/kl14.jpg'
import kl15 from '../assets/images/kl15.jpg'
import ko1 from '../assets/images/KONAAA.png'
import ko2 from '../assets/images/KONA.png'
import ko3 from '../assets/images/ko1.png'
import ko4 from '../assets/images/ko2.png'
import ko5 from '../assets/images/ko3.png'
import ko6 from '../assets/images/ko4.png'
import r1 from '../assets/images/REMIX.jpg'
import r2 from '../assets/images/r1.jpg'
import d1 from '../assets/images/DIVAA.jpg'
import d2 from '../assets/images/di1.jpg'
import d3 from '../assets/images/di2.jpg'
import d4 from '../assets/images/di3.jpg'
import v1 from '../assets/images/VIOLA.jpg'
import v2 from '../assets/images/v1.jpg'
import v3 from '../assets/images/v2.jpg'
import v4 from '../assets/images/v3.jpg'
import an1 from '../assets/images/ANCONAAA.jpg'
import an2 from '../assets/images/ANCONA.jpg'
import an3 from '../assets/images/anc.png'
import an4 from '../assets/images/anc2.png'
import an5 from '../assets/images/anc3.png'
import an6 from '../assets/images/anc4.png'
import e1 from '../assets/images/ELECTRA.jpg'
import e2 from '../assets/images/el1.jpg'
import e3 from '../assets/images/el2.jpg'
import e4 from '../assets/images/el3.jpg'
import mo1 from '../assets/images/MOET.jpg'
import mo2 from '../assets/images/m1.jpg'
import c1 from '../assets/images/CREW.jpg'
import c2 from '../assets/images/c1.png'
import c3 from '../assets/images/c2.jpg'
import c4 from '../assets/images/c3.png'
import c5 from '../assets/images/c4.jpg'
import c6 from '../assets/images/c5.jpg'
import si1 from '../assets/images/SILA.jpg'
import si2 from '../assets/images/si1.png'
import si3 from '../assets/images/si2.png'
import kona2 from '../assets/images/k2two.png'
import kona2one from '../assets/images/k2one.png'
import kona2two from '../assets/images/KONA2.png'
import visa1 from '../assets/images/VISA.png'
import visa2 from '../assets/images/vi1.png'
import miss from '../assets/images/MISSS.jpg'
import miss1 from '../assets/images/MISS.png'
import miss2 from '../assets/images/miss1.png'
import miss3 from '../assets/images/miss2.jpg'
import miss4 from '../assets/images/miss3.jpg'
import miss5 from '../assets/images/miss4.jpg'
import mora from '../assets/images/MORA.png'
import mora1 from '../assets/images/mora1.png'
import mora2 from '../assets/images/mora2.jpg'
import mora3 from '../assets/images/mora3.png'
import mora4 from '../assets/images/mora4.png'
import mora5 from '../assets/images/mora5.png'
import mora6 from '../assets/images/mora6.png'
import mora7 from '../assets/images/mora7.png'
import mora8 from '../assets/images/mora8.png'
import mora9 from '../assets/images/mora9.png'
import mora10 from '../assets/images/mora10.png'
import mora11 from '../assets/images/mora11.png'
import donna from '../assets/images/DONNA.png'
import donna1 from '../assets/images/donna1.png'
import donna2 from '../assets/images/donna2.png'
import donna3 from '../assets/images/donna3.jpg'
import foulard from '../assets/images/FOULARD.jpg'
import foulard2 from '../assets/images/foulard1.png'
import foulard3 from '../assets/images/foulardd.png'
import b1 from '../assets/images/BARCELONA.png'
import b2 from '../assets/images/b1.png'
import b3 from '../assets/images/b2.png'
import padova from '../assets/images/PADOVA.jpg'
import padova2 from '../assets/images/pa1.jpg'
import monza from '../assets/images/MONZA.jpg'
import monza2 from '../assets/images/monza1.jpg'
import milano from '../assets/images/MILANO.jpg'
import milano1 from '../assets/images/milano1.jpg'
import celia from '../assets/images/CELIA.jpg'
import celia1 from '../assets/images/celia1.jpg'
import lotus from '../assets/images/LOTUS.jpg'
import lotus1 from '../assets/images/lotus1.jpg'
import lotus2 from '../assets/images/lotus2.jpg'
import roma from '../assets/images/ROMA.jpg'
import roma1 from '../assets/images/roma1.jpg'
import relax from '../assets/images/RELAX.jpg'
import relax1 from '../assets/images/relax1.jpg'
import relax2 from '../assets/images/relax2.jpg'
import primo from '../assets/images/PRIMO.jpg'
import primo1 from '../assets/images/primo1.jpg'
import primo2 from '../assets/images/primo2.jpg'
import primo3 from '../assets/images/primo3.jpg'
import primo4 from '../assets/images/primo4.jpg'
import primo5 from '../assets/images/primo5.jpg'
import primo6 from '../assets/images/primo6.jpg'
import foulard22 from '../assets/images/FOULARDD.jpg'
import foulard23 from '../assets/images/fo1.jpg'
import foulard24 from '../assets/images/fo2.jpg'
import foulard25 from '../assets/images/fo3.jpg'
import foulard26 from '../assets/images/fo4.jpg'
import foulard27 from '../assets/images/fo5.jpg'
import foulard28 from '../assets/images/fo6.jpg'
import foulard29 from '../assets/images/fo7.jpg'
import theater from '../assets/images/THEATER.png'
import theater1 from '../assets/images/theater1.jpg'
import theater2 from '../assets/images/theater2.png'
import theater3 from '../assets/images/theater3.png'
import waiting from '../assets/images/wchairs.png'
import waiting1 from '../assets/images/wch1.jpg'
import waiting2 from '../assets/images/wch2.png'
import waiting3 from '../assets/images/wch3.png'
import waiting4 from '../assets/images/wch4.png'
import rea from '../assets/images/r004.jpg'
import rea1 from '../assets/images/r002.png'
import rea2 from '../assets/images/r003.jpg'
import rea3 from '../assets/images/r006.png'
import nice from '../assets/images/n001.jpg'
import nice1 from '../assets/images/n002.jpg'
import nice2 from '../assets/images/n003.png'
import nice3 from '../assets/images/n004.jpg'
import hera from '../assets/images/h001.png'
import hera1 from '../assets/images/h002.png'
import hera2 from '../assets/images/h003.png'
import hera3 from '../assets/images/h004.png'
import hermes from '../assets/images/h002.jpg'
import hermes1 from '../assets/images/h003.png'
import hermes2 from '../assets/images/h004.jpg'
import hermes3 from '../assets/images/h005.jpg'
import noble from '../assets/images/n002.jpg'
import noble1 from '../assets/images/n003.png'
import noble2 from '../assets/images/w003.jpg'
import mild from '../assets/images/m002.jpg'
import mild1 from '../assets/images/m001.jpg'
import mild2 from '../assets/images/m003.jpg'
import mild3 from '../assets/images/m004.png'
import lito from '../assets/images/l002.jpg'
import lito1 from '../assets/images/l001.jpg'
import lito2 from '../assets/images/l003.jpg'
import lito3 from '../assets/images/l004.jpg'
import lito4 from '../assets/images/l005.jpg'
import genius from '../assets/images/g001.jpg'
import genius1 from '../assets/images/g002.jpg'
import genius2 from '../assets/images/g003.jpg'
import genius3 from '../assets/images/g004.jpg'
import easy from '../assets/images/e001.jpg'
import easy1 from '../assets/images/e003.jpg'
import fors from '../assets/images/f001.jpg'
import fors1 from '../assets/images/f002.jpg'
import fors2 from '../assets/images/f003.jpg'
import truva from '../assets/images/t004.jpg'
import truva1 from '../assets/images/t001.jpg'
import truva2 from '../assets/images/t002.jpg'
import truva3 from '../assets/images/t003.jpg'
import window from '../assets/images/w001.jpg'
import window2 from '../assets/images/w002.jpg'
import artemis from '../assets/images/a003.jpg'
import artemis1 from '../assets/images/a001.png'
import artemis2 from '../assets/images/a002.jpg'
import prisma from '../assets/images/p002.jpg'
import prisma1 from '../assets/images/p001.jpg'
import optima from '../assets/images/o001.jpg'
import optima1 from '../assets/images/o002.jpg'
import optima2 from '../assets/images/o003.jpg'
import optima3 from '../assets/images/o004.jpg'
import kronos from '../assets/images/k002.jpg'
import kronos1 from '../assets/images/k001.jpg'
import uranus from '../assets/images/u002.jpg'
import uranus1 from '../assets/images/u001.jpg'
import others from '../assets/images/TP001.jpg'
import others1 from '../assets/images/TP002.jpg'
import others2 from '../assets/images/TP003.jpg'
import others3 from '../assets/images/TP004.jpg'
import others4 from '../assets/images/TPL001.jpg'
import bottom from '../assets/images/bottom.jpg'
import bottom1 from '../assets/images/bottom1.jpg'
import bottom2 from '../assets/images/bottom2.jpg'
import bottom3 from '../assets/images/bottom3.jpg'
import bottom4 from '../assets/images/bottom4.jpg'
import bottom5 from '../assets/images/bottom5.jpg'
import bottom6 from '../assets/images/bottom6.jpeg'
import bottom7 from '../assets/images/bottom7.jpg'
import bottom8 from '../assets/images/bottom8.jpg'
import bottom9 from '../assets/images/bottom9.jpg'
import bottom10 from '../assets/images/bottom10.jpg'
import bottom11 from '../assets/images/bottom11.jpg'
import bottom12 from '../assets/images/bottom12.jpg'
import bottom13 from '../assets/images/bottom13.jpeg'
import bottom14 from '../assets/images/bottom14.jpeg'
import bottom15 from '../assets/images/bottom15.jpeg'
import bottom16 from '../assets/images/bottom16.jpeg'
import bottom17 from '../assets/images/bottom17.jpeg'
import bottom18 from '../assets/images/bottom18.jpeg'
import bottom19 from '../assets/images/bottom19.jpeg'
import bottom20 from '../assets/images/bottom20.jpeg'
import bottom21 from '../assets/images/bottom21.jpeg'

import tika from '../assets/images/tika.jpg'
import tika2 from '../assets/images/tika2.jpg'
import tika3 from '../assets/images/tika3.jpg'
import tika4 from '../assets/images/tika4.jpg'
import tika5 from '../assets/images/tika5.jpg'
import tika6 from '../assets/images/tika6.jpg'
import tika7 from '../assets/images/tika7.jpg'
import tika8 from '../assets/images/tika8.jpg'
import tika9 from '../assets/images/tika9.jpg'
import tika10 from '../assets/images/tika10.jpg'
import tika11 from '../assets/images/tika11.jpg'
import tika12 from '../assets/images/tika12.jpg'



import usmile from '../assets/images/usmile1.jpg'
import usmile1 from '../assets/images/usmile1.jpg'
import usmile2 from '../assets/images/usmile2.jpg'
import usmile3 from '../assets/images/usmile3.jpg'
import usmile4 from '../assets/images/usmile4.jpg'
import usmile5 from '../assets/images/usmile5.jpg'
import usmile6 from '../assets/images/usmile6.jpg'
import usmile7 from '../assets/images/usmile7.jpg'
import usmile8 from '../assets/images/usmile8.jpg'
import usmile9 from '../assets/images/usmile9.jpg'
import usmile10 from '../assets/images/usmile10.jpg'
import usmile11 from '../assets/images/usmile11.jpg'
import usmile12 from '../assets/images/usmile12.jpg'
import usmile13 from '../assets/images/usmile13.jpg'
import usmile14 from '../assets/images/usmile14.jpg'
import usmile15 from '../assets/images/usmile15.jpg'




import integrime from '../assets/images/integrime.jpg'
import integrime1 from '../assets/images/integrime1.jpg'
import integrime2 from '../assets/images/integrime2.jpg'
import integrime3 from '../assets/images/integrime3.jpg'
import integrime4 from '../assets/images/integrime4.jpg'
import integrime5 from '../assets/images/integrime5.jpg'
import integrime6 from '../assets/images/integrime6.jpg'
import integrime7 from '../assets/images/integrime7.jpg'
import integrime8 from '../assets/images/integrime8.jpg'
import mile from '../assets/images/mile1.jpg'
import mile1 from '../assets/images/mile2.jpg'
import mile2 from '../assets/images/mile3.jpg'
import mile3 from '../assets/images/mile4.jpg'
import mile4 from '../assets/images/mile5.jpg'
import mile5 from '../assets/images/mile6.jpg'
import mile6 from '../assets/images/mile7.jpg'
import mile7 from '../assets/images/mile8.jpg'
import mile8 from '../assets/images/mile9.jpg'
import mile9 from '../assets/images/mile10.jpg'
import mile10 from '../assets/images/mile11.jpg'
import mile11 from '../assets/images/mile12.jpg'
import mile12 from '../assets/images/mile13.jpg'
import mile13 from '../assets/images/mile14.jpg'
import mile14 from '../assets/images/mile15.jpg'
import mile15 from '../assets/images/mile16.jpg'
import mile16 from '../assets/images/mile17.jpg'
import mile17 from '../assets/images/mile18.jpg'
import mile18 from '../assets/images/mile19.jpg'
import mile19 from '../assets/images/mile20.jpg'
import mile20 from '../assets/images/mile21.jpg'
import mile21 from '../assets/images/mile22.jpg'
import mile22 from '../assets/images/mile23.jpg'
import mile23 from '../assets/images/mile24.jpg'
import mile24 from '../assets/images/mile25.jpg'
import mile25 from '../assets/images/mile26.jpg'
import mile26 from '../assets/images/mile27.jpg'
import mile27 from '../assets/images/mile28.jpg'



import kultur from '../assets/images/kultur.jpg'
import kultur1 from '../assets/images/kultur1.jpg'
import kultur2 from '../assets/images/kultur2.jpg'
import kultur3 from '../assets/images/kultur3.jpg'
import kultur4 from '../assets/images/kultur4.jpg'
import kultur5 from '../assets/images/kultur5.jpg'
import kultur6 from '../assets/images/kultur6.jpg'
import kultur7 from '../assets/images/kultur7.jpg'
import kultur8 from '../assets/images/kultur8.jpg'
import kultur9 from '../assets/images/kultur9.jpg'
import kultur10 from '../assets/images/kultur10.jpg'
import kultur11 from '../assets/images/kultur11.jpg'
import kultur12 from '../assets/images/kultur12.jpg'
import kultur13 from '../assets/images/kultur13.jpg'
import kultur14 from '../assets/images/kultur14.jpg'
import kultur15 from '../assets/images/kultur15.jpg'
import kultur16 from '../assets/images/kultur16.jpg'
import kultur17 from '../assets/images/kultur17.jpg'
import kultur18 from '../assets/images/kultur18.jpg'


const kulturphoto = kultur;
const kulturphotos = [kultur1, kultur2, kultur3, kultur4, kultur5, kultur6, kultur7, kultur8, kultur9, kultur10, kultur11, kultur12,kultur13, kultur14, kultur15, kultur16, kultur17]
const integrimephoto = integrime;
const integrimephotos = [integrime1, integrime2, integrime3, integrime4, integrime5, integrime6, integrime7, integrime8]
const usmilephoto = usmile;
const usmilephotos = [usmile1, usmile2, usmile3, usmile4, usmile5, usmile6, usmile7, usmile8, usmile9, usmile10, usmile11 , usmile12, usmile13, usmile14, usmile15,]
const tikaphoto = tika;
const tikaphotos = [ tika2, tika3, tika4, tika5, tika6, tika7, tika8, tika9, tika10, tika11, tika12]
const mileniumphoto = mile;
const mileniumphotos = [mile1, mile2, mile3, mile4, mile5, mile6, mile7, mile8, mile9, mile10, mile11, mile12, mile13, mile14, mile15, mile16, mile17, mile18, mile19, mile20, mile21, mile22, mile23, mile24, mile15, mile26, mile27

]
const bottomphoto = bottom;
const bottomphotos = [bottom1, bottom2, bottom3, bottom4, bottom5, bottom6, bottom7, bottom8, bottom9, bottom10, bottom11, bottom12, bottom13, bottom14, bottom15, bottom16, bottom17, bottom18, bottom19, bottom20, bottom21]

const lightphoto = l1;
const lightphotos = [ l2, l3,l13,l15]
const giuliaphoto = g1;
const giuliaphotos=[g2,g3,g4,g5]
const jetphoto = j1;
const jetphotos = [ j2, j3, j4, j5]
const kingphoto = k1;
const kingphotos = [k2, k3]
const maliqephoto = m1;
const maliqephotos = [m2, m3]
const queenphoto = q1;
const queenphotos = [q2, q3, q4, q5]
const klaasphoto = kl1;
const klaasphotos = [kl2,  kl4, kl14,kl15]
const konaphoto = ko1;
const konaphotos = [ko2, ko3, ko4, ko5]
const remixphoto = r1;
const remixphotos = [ r2, r1]
const divaphoto = d1; 
const divaphotos = [d2,d3,d4]
const violaphoto= v1;
const violaphotos=[v2,v3,v4]
const anconaphoto = an1;
const anconaphotos = [an2, an4,an5,an6]
const electraphoto = e1;
const electraphotos = [e2,e3,e4]
const moetphoto = mo1;
const moetphotos = [mo1, mo2]
const crewphoto = c1;
const crewphotos = [c2, c3, c4,c5]
const silaphoto = si1;
const silaphotos = [si2, si3]
const kona2photo = kona2;
const kona2photos = [kona2one, kona2two]
const visaphoto = visa1;
const visaphotos = [visa2, visa1]
const missphoto = miss;
const missphotos = [miss1, miss2, miss3,  miss5]
const moraphoto = mora;
const moraphotos = [mora1, mora3, mora6,  mora11]
const donnaphoto = donna;
const donnaphotos = [donna1, donna2, donna3]
const foulardphoto = foulard;
const foulardphotos = [foulard2, foulard3]
const barcelonaphoto = b1;
const barcelonaphotos = [b2, b3]
const padovaphoto = padova;
const padovaphotos = [padova2, padova]
const monzaphoto = monza;
const monzaphotos = [monza2, monza]
const milanophoto = milano;
const milanophotos = [milano1, milano]
const celiaphoto = celia;
const celiaphotos = [celia1, celia]
const lotusphoto = lotus;
const lotusphotos = [lotus2, lotus1, lotus]
const romaphoto = roma;
const romaphotos = [roma1, roma]
const relaxphoto = relax;
const relaxphotos = [relax1, relax, relax2]
const primophoto = primo;
const primophotos = [primo1, primo2, primo4, primo5]
const foulard2photo = foulard22;
const foulard2photos = [foulard26, foulard27, foulard28, foulard29]
const theaterphoto = theater;
const theaterphotos = [theater1, theater2, theater3]
const waitingphoto = waiting;
const waitingphotos = [waiting1, waiting2,waiting3,waiting4]
const reaphoto = rea;
const reaphotos = [rea1, rea2, rea3]
const nicephoto = nice;
const nicephotos =[nice1, nice2, nice3]
const heraphoto = hera;
const heraphotos = [hera1, hera2, hera3]
const hermesphoto = hermes;
const hermesphotos = [hermes1, hermes2, hermes3]
const noblephoto = noble;
const noblephotos = [noble1, noble2]
const mildphoto = mild;
const mildphotos = [mild1, mild2,mild3]
const litophoto = lito;
const litophotos = [lito1, lito2, lito3, lito4]
const geniusphoto = genius;
const geniusphotos = [genius1, genius2, genius3]
const easyphoto = easy; 
const easyphotos = [easy1, easy]
const forsphoto = fors;
const forsphotos= [fors1,fors2]
const truvaphoto = truva;
const truvaphotos = [truva1, truva2,truva3]
const windowphoto =window;
const windowphotos = [window2, window]
const artemisphoto = artemis;
const artemisphotos = [artemis1, artemis2]
const prismaphoto = prisma;
const prismaphotos = [prisma1,prisma]
const optimaphoto = optima;
const optimaphotos = [optima1, optima2,optima3]
const kronosphoto = kronos;
const kronosphotos = [kronos1, kronos]
const uranusphoto = uranus;
const uranusphotos = [uranus1, uranus]
const otherphoto = others; 
const othersphotos = [others1, others2, others3 ,others4]
const codelight=["CH001", "CH002", "CH001", "CH002"]
const codegiulia=["CH001", "CH001", "CH002", "CH002"]
const codejet=["CH001", "CH001", "CH001", "CH001"]
const codeklaas=["CH001", "CH002", "CH001", "CH001"]
const codediva=["CH001", "CH002", "CH001"]
const codecrew=["CH001", "CH001", "CH001", "CH002"]
const codemora=["CH001", "CH002", "CH003", "CH004"]
const codelotus=["CH001","CH002", "CH002"]










const dataProducts = {
    "sq": {
        "light":{

            category: "Karrigë Zyreje",
            name: "Light",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: lightphotos,
            firstphoto:lightphoto,
            name2:codelight
        },
        "giulia":{

            category: "Karrigë Zyreje",
            name: "Giulia",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: giuliaphotos,
            firstphoto:giuliaphoto,
            name2:codegiulia

        },
        "jet":{

            category: "Karrigë Zyreje",
            name: "Jet",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: jetphotos,
            firstphoto:jetphoto,
            name2:codejet

        },
        "king":{

            category: "Karrigë Zyreje",
            name: "King",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: kingphotos,
            firstphoto:kingphoto,
            name2:codejet

        },
        "maliqe":{

            category: "Karrigë Zyreje",
            name: "Malice",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: maliqephotos,
            firstphoto:maliqephoto,
            name2:codejet

        },
        "queen":{

            category: "Karrigë Zyreje",
            name: "Queen",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: queenphotos,
            firstphoto:queenphoto,
            name2:codelight

        },
        "klaas":{

            category: "Karrigë Zyreje",
            name: "Klaas",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: klaasphotos,
            firstphoto:klaasphoto,
            name2:codeklaas

        },
        "kona":{

            category: "Karrigë Zyreje",
            name: "Kona",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: konaphotos,
            firstphoto:konaphoto,
            name2:codelight

        },
        "remix":{

            category: "Karrigë Zyreje",
            name: "Remix",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: remixphotos,
            firstphoto:remixphoto,
            name2:codejet

        },
        "diva":{

            category: "Karrigë Zyreje",
            name: "Diva",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: divaphotos,
            firstphoto:divaphoto,
            name2:codediva

        },
        "viola":{

            category: "Karrigë Zyreje",
            name: "Viola",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: violaphotos,
            firstphoto:violaphoto,
            name2:codegiulia

        },
        "ancona":{

            category: "Karrigë Zyreje",
            name: "Ancona",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: anconaphotos,
            firstphoto:anconaphoto,
            name2:codegiulia

        },
        "electra":{

            category: "Karrigë Zyreje",
            name: "Electra",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: electraphotos,
            firstphoto:electraphoto,
            name2:codegiulia

        },
        "moet": {
            category: "Karrigë Zyreje",
            name: "Moet",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: moetphotos,
            firstphoto:moetphoto,
            name2:codelight

        },
        "crew": {
            category: "Karrigë takimesh",
            name: "Crew",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: crewphotos,
            firstphoto:crewphoto,
            name2:codecrew

        },
        "sila": {
            category: "Karrigë takimesh",
            name: "Sila",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: silaphotos,
            firstphoto:silaphoto,
            name2:codelight

        },
        "kona2": {
            category: "Karrigë takimesh",
            name: "Kona",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: kona2photos,
            firstphoto:kona2photo,
            name2:codejet

        },
        "visa": {
            category: "Karrigë takimesh",
            name: "Visa",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: visaphotos,
            firstphoto:visaphoto,
            name2:codelight

        },
        "miss": {
            category: "Karrigë takimesh",
            name: "Miss",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: missphotos,
            firstphoto:missphoto,
            name2:codegiulia

        },
        "mora": {
            category: "Karrigë takimesh",
            name: "Mora",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: moraphotos,
            firstphoto:moraphoto,
            name2:codemora

        },
        "donna": {
            category: "Karrigë takimesh",
            name: "Donna",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: donnaphotos,
            firstphoto:donnaphoto,
            name2:codegiulia

        },
        "foulard": {
            category: "Karrigë takimesh",
            name: "Foulard",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: foulardphotos,
            firstphoto:foulardphoto,
            name2:codegiulia

        },
        "barcelona": {
            category: "Karrigë Pritjeje",
            name: "Barcelona",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: barcelonaphotos,
            firstphoto:barcelonaphoto,
            name2:codelight

        },
        "padova": {
            category: "Karrigë Pritjeje",
            name: "Padova",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: padovaphotos,
            firstphoto:padovaphoto,
            name2:codegiulia

        },
        "monza": {
            category: "Karrigë Pritjeje",
            name: "Monza",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: monzaphotos,
            firstphoto:monzaphoto,
            name2:codelight

        },
        "milano": {
            category: "Karrigë Pritjeje",
            name: "Milano",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: milanophotos,
            firstphoto:milanophoto,
            name2:codelight

        },
        "celia": {
            category: "Karrigë Pritjeje",
            name: "Celia",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: celiaphotos,
            firstphoto:celiaphoto,
            name2:codelight

        },
        "lotus": {
            category: "Karrigë Pritjeje",
            name: "Lotus",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: lotusphotos,
            firstphoto:lotusphoto,
            name2:codelotus

        },
        "roma": {
            category: "Karrigë Pritjeje",
            name: "Roma",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: romaphotos,
            firstphoto:romaphoto,
            name2:codelotus

        },
        "relax": {
            category: "Karrigë Pritjeje",
            name: "Relax",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: relaxphotos,
            firstphoto:relaxphoto,
            name2:codemora

        },
        "primo": {
            category: "Karrigë Pritjeje",
            name: "Primo",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: primophotos,
            firstphoto:primophoto,
            name2:codegiulia

        },
        "foulard22": {
            category: "Karrigë Pritjeje",
            name: "Foulard",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: foulard2photos,
            firstphoto:foulard2photo,
            name2:codejet

        },
        "theater": {
            category: "Karrigë Pritjeje",
            name: "Theater",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: theaterphotos,
            firstphoto:theaterphoto,
            name2:codegiulia

        },
        "waiting": {
            category: "Karrigë Pritjeje",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: waitingphotos,
            firstphoto:waitingphoto,
            name2:codegiulia

        },
        "rea":{
            category: "Tavolinë Pune",
            name: "Rea",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: reaphotos,
            firstphoto:reaphoto,
           // name2:codegiulia

        },
        "nice":{
            category: "Tavolinë Pune",
            name: "Nice",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: nicephotos,
            firstphoto:nicephoto
        },
        "hera":{
            category: "Tavolinë Pune",
            name: "Hera",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: heraphotos,
            firstphoto:heraphoto
        },
        "hermes":{
            category: "Tavolinë Pune",
            name: "Hermes",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: hermesphotos,
            firstphoto:hermesphoto
        },
        "noble":{
            category: "Tavolinë Pune",
            name: "Noble",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: noblephotos,
            firstphoto:noblephoto
        },
        "mild":{
            category: "Tavolinë Pune",
            name: "Working Tables",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: mildphotos,
            firstphoto:mildphoto
        },
        "lito":{
            category: "Tavolinë Pune",
            name: "Lito",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: litophotos,
            firstphoto:litophoto
        },
        "genius":{
            category: "Tavolinë Pune",
            name: "Genius",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: geniusphotos,
            firstphoto:geniusphoto
        },
        "easy":{
            category: "Tavolinë Pune",
            name: "Easy",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: easyphotos,
            firstphoto:easyphoto
        },
        "fors":{
            category: "Tavolinë Pune",
            name: "Fors",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: forsphotos,
            firstphoto:forsphoto
        },
        "truva":{
            category: "Tavolinë Pune",
            name: "Truva",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: truvaphotos,
            firstphoto:truvaphoto
        },
        "window":{
            category: "Tavolinë Pune",
            name: "Truva Window",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: windowphotos,
            firstphoto:windowphoto
        },
        "artemis":{
            category: "Tavolinë Pune",
            name: "Artemis",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: artemisphotos,
            firstphoto:artemisphoto
        },
        "prisma":{
            category: "Tavolinë Pune",
            name: "Prisma",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: prismaphotos,
            firstphoto:prismaphoto
        },
        "optima":{
            category: "Tavolinë Pune",
            name: "Optima",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: optimaphotos,
            firstphoto:optimaphoto
        },
        "kronos":{
            category: "Tavolinë Pune",
            name: "Kronos",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: kronosphotos,
            firstphoto:kronosphoto
        },
        "uranus":{
            category: "Tavolinë Pune",
            name: "Uranus",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: uranusphotos,
            firstphoto:uranusphoto
        },
        "others":{
            category: "Tavolina Pune",
            name: "Të tjera",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: othersphotos,
            firstphoto:otherphoto
        },
        "bottomline prishtina and geneva":{
            photos:bottomphotos,
            firstphoto:bottomphoto

        },
        "millennium challenge account kosovo":{
            photos:mileniumphotos,
            firstphoto: mileniumphoto

        },
        "tika - qendra për rehabilitimin dhe edukimin special":{

            photos:tikaphotos,
            firstphoto:tikaphoto
        },
        "u-smile office - liège":{
            photos:usmilephotos,
            firstphoto:usmilephoto

        },
        "ministria e integrimeve - republika e kosovës":{
            photos:integrimephotos,
            firstphoto:integrimephoto
        },
        "ministria e kulturës - republika e kosovës":{
            photos:kulturphotos,
            firstphoto:kulturphoto
        }

    },
    "en": {
        "light":{

            category: "Office Chair",
            name: "Light",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: lightphotos,
            firstphoto:lightphoto,
            name2:codelight

        },
        "giulia":{

            category: "Office Chair",
            name: "Giulia",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: giuliaphotos,
            firstphoto:giuliaphoto,
            name2:codegiulia

        },
        "jet":{

            category: "Office Chair",
            name: "Jet",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: jetphotos,
            firstphoto:jetphoto,
            name2:codejet

        },
        "king":{

            category: "Office Chair",
            name: "King",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: kingphotos,
            firstphoto:kingphoto,
            name2:codejet

        },
        "maliqe":{

            category: "Office Chair",
            name: "Malice",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: maliqephotos,
            firstphoto:maliqephoto,
            name2:codejet

        },
        "queen":{

            category: "Office Chair",
            name: "Queen",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: queenphotos,
            firstphoto:queenphoto,
            name2:codelight

        },
        "klaas":{

            category: "Office Chair",
            name: "Klaas",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: klaasphotos,
            firstphoto:klaasphoto,
            name2:codeklaas

        },
        "kona":{

            category: "Office Chair",
            name: "Kona",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: konaphotos,
            firstphoto:konaphoto,
            name2:codelight

        },
        "remix":{

            category: "Office Chair",
            name: "Remix",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: remixphotos,
            firstphoto:remixphoto,
            name2:codejet

        },
        "diva":{

            category: "Office Chair",
            name: "Diva",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: divaphotos,
            firstphoto:divaphoto,
            name2:codediva

        },
        "viola":{

            category: "Office Chair",
            name: "Viola",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: violaphotos,
            firstphoto:violaphoto,
            name2:codegiulia
        },
        "ancona":{

            category: "Office Chair",
            name: "Ancona",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: anconaphotos,
            firstphoto:anconaphoto,
            name2:codegiulia

        },
        "electra":{

            category: "Office Chair",
            name: "Electra",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: electraphotos,
            firstphoto:electraphoto,
            name2:codegiulia

        },
        "moet": {
            category: "Office Chair",
            name: "Moet",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: moetphotos,
            firstphoto:moetphoto,
            name2:codelight
        },
        "crew": {
            category: "Meeting Chair",
            name: "Crew",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: crewphotos,
            firstphoto:crewphoto,
            name2:codecrew,


        },
        "sila": {
            category: "Meeting Chair",
            name: "Sila",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: silaphotos,
            firstphoto:silaphoto,
            name2:codelight

        },
        "kona2": {
            category: "Meeting Chair",
            name: "Kona",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: kona2photos,
            firstphoto:kona2photo,
            name2:codejet

        },
        "visa": {
            category: "Meeting Chair",
            name: "Visa",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: visaphotos,
            firstphoto:visaphoto,
            name2:codelight

        },
        "miss": {
            category: "Meeting Chair",
            name: "Miss",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: missphotos,
            firstphoto:missphoto,
            name2:codegiulia
        },
        "mora": {
            category: "Meeting Chair",
            name: "Mora",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: moraphotos,
            firstphoto:moraphoto,
            name2:codemora
        },
        "donna": {
            category: "Meeting Chair",
            name: "Donna",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: donnaphotos,
            firstphoto:donnaphoto,
            name2:codegiulia

        },
        "foulard": {
            category: "Meeting Chair",
            name: "Foulard",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: foulardphotos,
            firstphoto:foulardphoto,
            name2:codegiulia
        },
        "barcelona": {
            category: "Waiting Chair",
            name: "Barcelona",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: barcelonaphotos,
            firstphoto:barcelonaphoto,
            name2:codelight
        },
        "padova": {
            category: "Waiting Chair",
            name: "Padova",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: padovaphotos,
            firstphoto:padovaphoto,
            name2:codegiulia
        },
        "monza": {
            category: "Waiting Chair",
            name: "Monza",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: monzaphotos,
            firstphoto:monzaphoto,
            name2:codelight
        },
        "milano": {
            category: "Waiting Chair",
            name: "Milano",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: milanophotos,
            firstphoto:milanophoto,
            name2:codelight
        },
        "celia": {
            category: "Waiting Chair",
            name: "Celia",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: celiaphotos,
            firstphoto:celiaphoto,
            name2:codelight
        },
        "lotus": {
            category: "Waiting Chair",
            name: "Lotus",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: lotusphotos,
            firstphoto:lotusphoto,
            name2:codelotus
        },
        "roma": {
            category: "Waiting Chair",
            name: "Roma",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: romaphotos,
            firstphoto:romaphoto,
            name2:codelotus
        },
        "relax": {
            category: "Waiting Chair",
            name: "Relax",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: relaxphotos,
            firstphoto:relaxphoto,
            name2:codemora
        },
        "primo": {
            category: "Waiting Chair",
            name: "Primo",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: primophotos,
            firstphoto:primophoto,
            name2:codegiulia
        },
        "foulard22": {
            category: "Waiting Chair",
            name: "Foulard",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: foulard2photos,
            firstphoto:foulard2photo,
            name2:codejet

        },
        "theater": {
            category: "Waiting Chair",
            name: "Theater",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: theaterphotos,
            firstphoto:theaterphoto,
            name2:codegiulia
        },
        "waiting": {
            category: "Waiting Chair",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: waitingphotos,
            firstphoto:waitingphoto,
            name2:codegiulia
        }, 
        "rea":{
            category: "Working Table",
            name: "Rea",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: reaphotos,
            firstphoto:reaphoto
        },
        "nice":{
            category: "Working Table",
            name: "Nice",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: nicephotos,
            firstphoto:nicephoto
        },
        "hera":{
            category: "Working Table",
            name: "Hera",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: heraphotos,
            firstphoto:heraphoto
        },
        "hermes":{
            category: "Working Table",
            name: "Hermes",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: hermesphotos,
            firstphoto:hermesphoto
        },
        "noble":{
            category: "Working Table",
            name: "Noble",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: noblephotos,
            firstphoto:noblephoto
        },
        "mild":{
            category: "Working Table",
            name: "Mild",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: mildphotos,
            firstphoto:mildphoto
        },
        "lito":{
            category: "Working Table",
            name: "Lito",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: litophotos,
            firstphoto:litophoto
        },
        "genius":{
            category: "Working Table",
            name: "Genius",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: geniusphotos,
            firstphoto:geniusphoto
        },
        "easy":{
            category: "Working Table",
            name: "Easy",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: easyphotos,
            firstphoto:easyphoto
        },
        "fors":{
            category: "Working Table",
            name: "Fors",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: forsphotos,
            firstphoto:forsphoto
        },
        "truva":{
            category: "Working Table",
            name: "Truva",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: truvaphotos,
            firstphoto:truvaphoto
        },
        "window":{
            category: "Working Table",
            name: "Truva Window",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: windowphotos,
            firstphoto:windowphoto
        },
        "artemis":{
            category: "Working Table",
            name: "Artemis",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: artemisphotos,
            firstphoto:artemisphoto
        },
        "prisma":{
            category: "Working Table",
            name: "Prisma",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: prismaphotos,
            firstphoto:prismaphoto
        },
        "optima":{
            category: "Working Table",
            name: "Optima",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: optimaphotos,
            firstphoto:optimaphoto
        },
        "kronos":{
            category: "Working Table",
            name: "Kronos",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: kronosphotos,
            firstphoto:kronosphoto
        },
        "uranus":{
            category: "Working Table",
            name: "Uranus",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: uranusphotos,
            firstphoto:uranusphoto
        },
        "others":{
            category: "Working Tables",
            name: "Others",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: othersphotos,
            firstphoto:otherphoto
        },
        "bottomline prishtina and geneva":{
            photos:bottomphotos,
            firstphoto:bottomphoto
        },
        "millennium challenge account kosovo":{
            photos:mileniumphotos,
            firstphoto: mileniumphoto

        },
        "tika - qendra për rehabilitimin dhe edukimin special":{

            photos:tikaphotos,
            firstphoto:tikaphoto
        },
        "u-smile office - liège":{
            photos:usmilephotos,
            firstphoto:usmilephoto

        },
        "ministria e integrimeve - republika e kosovës":{
            photos:integrimephotos,
            firstphoto:integrimephoto
        },
        "ministria e kulturës - republika e kosovës":{
            photos:kulturphotos,
            firstphoto:kulturphoto
        }

    },
    "de": {
        "light":{

            category: "Büro Stuhl",
            name: "Light",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: lightphotos,
            firstphoto:lightphoto,
            name2:codelight

        },
        "giulia":{

            category: "Büro Stuhl",
            name: "Giulia",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: giuliaphotos,
            firstphoto:giuliaphoto,
            name2:codegiulia
        },
        "jet":{

            category: "Büro Stuhl",
            name: "Jet",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: jetphotos,
            firstphoto:jetphoto,
            name2:codejet
        },
        "king":{

            category: "Büro Stuhl",
            name: "King",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: kingphotos,
            firstphoto:kingphoto,
            name2:codejet
        },
        "maliqe":{

            category: "Büro Stuhl",
            name: "Malice",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: maliqephotos,
            firstphoto:maliqephoto,
            name2:codejet
        },
        "queen":{

            category: "Büro Stuhl",
            name: "Queen",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: queenphotos,
            firstphoto:queenphoto,
            name2:codelight
        },
        "klaas":{

            category: "Büro Stuhl",
            name: "Klaas",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: klaasphotos,
            firstphoto:klaasphoto,
            name2:codeklaas
        },
        "kona":{

            category: "Büro Stuhl",
            name: "Kona",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: konaphotos,
            firstphoto:konaphoto,
            name2:codelight
        },
        "remix":{

            category: "Büro Stuhl",
            name: "Remix",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: remixphotos,
            firstphoto:remixphoto,
            name2:codejet
        },
        "diva":{

            category: "Büro Stuhl",
            name: "Diva",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: divaphotos,
            firstphoto:divaphoto,
            name2:codediva
        },
        "viola":{

            category: "Büro Stuhl",
            name: "Viola",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: violaphotos,
            firstphoto:violaphoto,
            name2:codegiulia
        },
        "ancona":{

            category: "Büro Stuhl",
            name: "Ancona",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: anconaphotos,
            firstphoto:anconaphoto,
            name2:codegiulia

        },
        "electra":{

            category: "Büro Stuhl",
            name: "Electra",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: electraphotos,
            firstphoto:electraphoto,
            name2:codegiulia

        },
        "moet": {
            category: "Büro Stuhl",
            name: "Moet",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: moetphotos,
            firstphoto:moetphoto,
            name2:codelight
        },
        "crew": {
            category: "Besprechungs Stuhl",
            name: "Crew",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: crewphotos,
            firstphoto:crewphoto,
            name2:codecrew

        },
        "sila": {
            category: "Besprechungs Stuhl",
            name: "Sila",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: silaphotos,
            firstphoto:silaphoto,
            name2:codelight

        },
        "kona2": {
            category: "Besprechungs Stuhl",
            name: "Kona",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: kona2photos,
            firstphoto:kona2photo,
            name2:codejet

        },
        "visa": {
            category: "Besprechungs Stuhl",
            name: "Visa",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: visaphotos,
            firstphoto:visaphoto
        },
        "miss": {
            category: "Besprechungs Stuhl",
            name: "Miss",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: missphotos,
            firstphoto:missphoto
        },
        "mora": {
            category: "Besprechungs Stuhl",
            name: "Mora",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: moraphotos,
            firstphoto:moraphoto
        },
        "donna": {
            category: "Besprechungs Stuhl",
            name: "Donna",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: donnaphotos,
            firstphoto:donnaphoto
        },
        "foulard": {
            category: "Besprechungs Stuhl",
            name: "Foulard",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: foulardphotos,
            firstphoto:foulardphoto
        },
        "barcelona": {
            category: "Warte Stuhl",
            name: "Barcelona",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: barcelonaphotos,
            firstphoto:barcelonaphoto
        },
        "padova": {
            category: "Warte Stuhl",
            name: "Padova",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: padovaphotos,
            firstphoto:padovaphoto
        },
        "monza": {
            category: "Warte Stuhl",
            name: "Monza",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: monzaphotos,
            firstphoto:monzaphoto
        },
        "milano": {
            category: "Warte Stuhl",
            name: "Milano",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: milanophotos,
            firstphoto:milanophoto
        },
        "celia": {
            category: "Warte Stuhl",
            name: "Celia",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: celiaphotos,
            firstphoto:celiaphoto
        },
        "lotus": {
            category: "Warte Stuhl",
            name: "Lotus",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: lotusphotos,
            firstphoto:lotusphoto
        },
        "roma": {
            category: "Warte Stuhl",
            name: "Roma",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: romaphotos,
            firstphoto:romaphoto
        },
        "relax": {
            category: "Warte Stuhl",
            name: "Relax",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: relaxphotos,
            firstphoto:relaxphoto
        },
        "primo": {
            category: "Warte Stuhl",
            name: "Primo",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: primophotos,
            firstphoto:primophoto
        },
        "foulard22": {
            category: "Warte Stuhl",
            name: "Foulard",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: foulard2photos,
            firstphoto:foulard2photo
        },
        "theater": {
            category: "Warte Stuhl",
            name: "Theater",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: theaterphotos,
            firstphoto:theaterphoto
        },
        "waiting": {
            category: "Warte Stuhl",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: waitingphotos,
            firstphoto:waitingphoto
        },
        "rea":{
            category: "Arbeits Tisch",
            name: "Rea",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: reaphotos,
            firstphoto:reaphoto
        },
        "nice":{
            category: "Arbeits Tisch",
            name: "Nice",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: nicephotos,
            firstphoto:nicephoto
        },
        "hera":{
            category: "Arbeits Tisch",
            name: "Hera",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: heraphotos,
            firstphoto:heraphoto
        },
        "hermes":{
            category: "Arbeits Tisch",
            name: "Hermes",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: hermesphotos,
            firstphoto:hermesphoto
        },
        "noble":{
            category: "Arbeits Tisch",
            name: "Noble",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: noblephotos,
            firstphoto:noblephoto
        },
        "mild":{
            category: "Arbeits Tisch",
            name: "Mild",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: mildphotos,
            firstphoto:mildphoto
        },
        "lito":{
            category: "Arbeits Tisch",
            name: "Lito",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: litophotos,
            firstphoto:litophoto
        },
        "genius":{
            category: "Arbeits Tisch",
            name: "Genius",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: geniusphotos,
            firstphoto:geniusphoto
        },
        "easy":{
            category: "Arbeits Tisch",
            name: "Easy",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: easyphotos,
            firstphoto:easyphoto
        },
        "fors":{
            category: "Arbeits Tisch",
            name: "Fors",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: forsphotos,
            firstphoto:forsphoto
        },
        "truva":{
            category: "Arbeits Tisch",
            name: "Truva",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: truvaphotos,
            firstphoto:truvaphoto
        },
        "window":{
            category: "Arbeits Tisch",
            name: "Truva Window",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: windowphotos,
            firstphoto:windowphoto
        },
        "artemis":{
            category: "Arbeits Tisch",
            name: "Artemis",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: artemisphotos,
            firstphoto:artemisphoto
        },
        "prisma":{
            category: "Arbeits Tisch",
            name: "Prisma",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: prismaphotos,
            firstphoto:prismaphoto
        },
        "optima":{
            category: "Arbeits Tisch",
            name: "Optima",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: optimaphotos,
            firstphoto:optimaphoto
        },
        "kronos":{
            category: "Arbeits Tisch",
            name: "Kronos",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: kronosphotos,
            firstphoto:kronosphoto
        },
        "uranus":{
            category: "Arbeits Tisch",
            name: "Uranus",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: uranusphotos,
            firstphoto:uranusphoto
        },
        "others":{
            category: "Arbeits Tische",
            name: "Andere",
            description: "I formuar nga traditat shekullore daneze, stili modern skandinav pasqyrohet në shtratin Grenen, i quajtur sipas pelerinës në pikën më veriore të vendit. Ndërsa Deti Baltik takohet me Veriun këtu, kështu komoditeti dhe komoditeti gjenden me cilësinë dhe besueshmërinë e shtratit Grenen.",
            photos: othersphotos,
            firstphoto:otherphoto
        },
        "bottomline prishtina and geneva":{
            photos:bottomphotos,
            firstphoto:bottomphoto
        },
        "millennium challenge account kosovo":{
            photos:mileniumphotos,
            firstphoto: mileniumphoto

        },
        "tika - qendra për rehabilitimin dhe edukimin special":{

            photos:tikaphotos,
            firstphoto:tikaphoto
        },
        "u-smile office - liège":{
            photos:usmilephotos,
            firstphoto:usmilephoto

        },
        "ministria e integrimeve - republika e kosovës":{
            photos:integrimephotos,
            firstphoto:integrimephoto
        },
        "ministria e kulturës - republika e kosovës":{
            photos:kulturphotos,
            firstphoto:kulturphoto
        }

    }
}

export {
    dataProducts
};
